...
contract("Netflix", (accounts) => {
    ...
    it("simulates a whole transaction", async () => {
        await Netflix.deployed().then(async function(contractInstance) {
            console.log("\n\nStarting test");
            let avail = await contractInstance.viewAvailItems({from: buyer});
            console.log(avail);
            console.log("\nListing new item.")
            await contractInstance.listItem("Screen 1", "new Netflix Screen", 100000, 1,{from: seller});
            await contractInstance.listItem("Screen 2", "The Best Netflix Screen", 500000, 1,{from: seller});
